...
- **Data is split into a client `core` and a server-only `full` dataset.** `src/lib/restaurants.generated.json` is the **full** source of truth (every field). `scripts/split-data.ts` derives `src/lib/restaurants.core.json` = the full dataset **minus the detail-only editorial fields `insiderTip` and `blurb`** — those are ~25% of the payload and only the detail page uses them. Both files are committed; **do not hand-edit either — regenerate with `npm run ingest`** (or `npm run split-data` to re-derive only core).
- **`src/lib/data.ts`** (client-safe) — exports `RESTAURANTS` by importing `restaurants.core.json`, plus `RESTAURANTS_BY_ID` (O(1) id index), `getRestaurant`, and `ALL_CUISINES`. Records here have **no `insiderTip`/`blurb`** (both optional on the `Restaurant` type).
...
- **Feed model:** the feed is an **editorial list of one `SpotCard` per restaurant** (not per reel). `SpotCard` is deliberately minimal (photo · gem score · name · one metadata line · save) and **`React.memo`'d** (the feed re-renders on every store mutation; without memo a save toggle re-renders every card). `src/components/Feed.tsx` **paginates** (renders 24, "Show more" reveals more) so we never mount ~1.6k cards / fire ~1.6k photo requests at once; the "you're caught up" end state shows once everything is revealed. Card images use `loading="lazy"`. **All depth lives on the detail page**: the "why you" reasons (recomputed via `scoreRestaurant`), insider tip, earliness cue, and full actions including `ShareSpot`. Preserve this progressive-disclosure split — do not re-clutter the card.
