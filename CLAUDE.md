...
- **AI concierge** (`src/app/api/assistant/route.ts`) — builds a candidate pool with the local engine, then calls Claude **if `ANTHROPIC_API_KEY` is set** (`ANTHROPIC_MODEL` optional), else returns a deterministic local result. The feature must always work without a key.
