...
- **Horizontal scroll memory** — Discover carousels now restore their scroll position
  when you tap a spot and press back (previously only vertical page scroll was remembered).
  Generalized `src/lib/useScrollRestoration.ts` with an `axis: "vertical" | "horizontal"`
  param; each `Shelf` keeps its `scrollLeft` keyed by shelf.
- **Collapsible "What to order"** — each dish is now a tappable accordion row showing just
  the name (plus a ◷ allergen dot when relevant); the why/note/caution expand on tap.
  Decluttered the densest block on the detail page (`OrderGuide.tsx`).
- **Pre-stored dish descriptions (Ordering Phase 3, hybrid)** — replaced the request-time
  `/api/order` Claude upgrade (which caused a ~6–7s delay and a visible pick reorder) with
  a rich, **pre-stored, dish-centric** description per dish, rendered instantly, plus the
  existing instant local taste line. New detail-only field
  `Restaurant.dishDescriptions`, generated by `generateDishDescriptions()` in
  `scripts/editorial.ts` and populated by a surgical, idempotent
  `scripts/enrich-dish-descriptions.ts` pass (`npm run enrich-dishes`) — no full re-ingest,
  so existing editorial copy doesn't churn. Dormant until the pass runs.

Decision: [`decisions/2026-06-21-prestored-dish-descriptions.md`](decisions/2026-06-21-prestored-dish-descriptions.md).
